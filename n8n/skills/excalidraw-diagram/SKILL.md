...
From this skill’s `references/` directory. On this machine the skill lives at `~/.cursor/skills/excalidraw-diagram` (Claude Code / project installs often use `.claude/skills/excalidraw-diagram` instead—use whichever matches your layout).

```bash
cd ~/.cursor/skills/excalidraw-diagram/references && .venv/bin/python render_excalidraw.py <path-to-file.excalidraw>
```
...
### First-Time Setup
If the render script hasn't been set up yet (from `references/` — adjust the path if your skill is not under `~/.cursor/skills/`):
```bash
...
```
If you use [uv](https://docs.astral.sh/uv/), you can run `uv sync` and `uv run playwright install chromium` instead (see the skill’s `README.md`).
