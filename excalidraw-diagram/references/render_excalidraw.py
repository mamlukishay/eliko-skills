...
Usage:
    cd <this-directory>
    .venv/bin/python render_excalidraw.py <path-to-file.excalidraw> [--output path.png] [--scale 2] [--width 1920]
...
Or with uv: uv sync && uv run playwright install chromium
"""
...
def render(
    ...
    try:
        ...
        print(
            f"Run: cd {_REF_DIR} && python3 -m venv .venv && .venv/bin/pip install playwright && .venv/bin/playwright install chromium",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print(f"Run: cd {_REF_DIR} && .venv/bin/playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
